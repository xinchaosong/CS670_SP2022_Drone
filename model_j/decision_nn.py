...
json_path = Path(__file__).resolve().parent / 'MODEL_1_Apr4_2022.json'
model_path = Path(__file__).resolve().parent / 'MODEL_1_Apr4_2022.h5'
...
class BoxDetector:
    ...
    def __load_model__(self):
        ...
        try:
            loaded_model_object.load_weights(model_path)
        except:
            raise Exception(
    ...
    def make_decision(self, frame):
        ...
        for x, y, w, h in self.allBoxes_Final:
            if 0.8 < w / h < 1.25:
                self.box_recognized = (x, y, w, h)

        if self.box_recognized is None:
            return ""

        x, y, w, h = self.box_recognized

        center_x = x + 0.5 * w
        center_y = y + 0.5 * h

        print("Box center: (%f, %f)" % (center_x, center_y))
        print("Box size: (%f, %f)" % (w, h))

        if w > 0.7 * frame_width:
            return "stop"

        elif center_x < 0.4 * frame_width:
            return "left"

        elif center_x > 0.6 * frame_width:
            return "right"

        elif center_y < 0.4 * frame_height:
            return "up"

        elif center_y > 0.6 * frame_width:
            return "down"
